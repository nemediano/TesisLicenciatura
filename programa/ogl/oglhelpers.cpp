...
void printVector(const glm::vec4& u) {
  cout << toStr(u) << endl;
}

void printVector(const glm::vec3& u) {
  cout << toStr(u) << endl;
}

std::string toStr(const glm::vec3& u) {
	std::stringstream info;
	info << "(" << u.x << ", " << u.y << ", " << u.z << ")";
	return info.str();
}

std::string toStr(const glm::vec4& u) {
	std::stringstream info;
	info << "(" << u.x << ", " << u.y << ", " << u.z << ", " << u.x << ")";
	return info.str();
}

std::string toStr(const glm::mat3& M) {
	std::stringstream info;
	info << M[0][0] << "\t" << M[0][1] << "\t" << M[0][2] << std::endl;
	info << M[1][0] << "\t" << M[1][1] << "\t" << M[1][2] << std::endl;
	info << M[2][0] << "\t" << M[2][1] << "\t" << M[2][2];
	return info.str();
}

std::string toStr(const glm::mat4& M) {
  std::stringstream info;
  info << M[0][0] << "\t" << M[0][1] << "\t" << M[0][2] << "\t" << M[0][3] << std::endl;
  info << M[1][0] << "\t" << M[1][1] << "\t" << M[1][2] << "\t" << M[1][3] << std::endl;
  info << M[2][0] << "\t" << M[2][1] << "\t" << M[2][2] << "\t" << M[2][3] << std::endl;
  info << M[3][0] << "\t" << M[3][1] << "\t" << M[3][2] << "\t" << M[3][3];
  return info.str();
}
